...
//
// Эта функция находит и решает все коллизии нашего игрового мира. Делает
// она это, попарно перебирая все объекты.
//
// Сначала пройдёмся в цикле по всем объектам сцены (scene). Это можно очень
// удобно сделать сделать с помощью цикла for-each, который появился в
// С++11: for (Object &obj1 : scene). Для каждого объекта проверим, что он
// подчиняется физическим законам нашего игрового мира. Для этого у него
// должен быть коллайдер (obj1.collider.enabled) - это некий прямоугольник,
// описывающий физические границы объекта. Кроме того, этот коллайдер должен
// быть динамическим (ColliderType::DYNAMIC), то есть объект может двигаться
// сам или под воздействием внешних сил, это можно проверить, используя
// метод of_type для obj.collider.
//
// Если наш объект удовлетворяет условиям, описанным выше, точно так же
// переберём все объекты сцены в качестве второго объекта, коллизию с
// которым и будем проверять. Убедимся в следующих вещах:
...
//
// Эта функция должна применять эффект гравитации к конкретному объекту
// (obj). Сначала нужно проверить, что полученный объект поклоняется
// физическим законам нашего игрового мира. Для этого нужно, чтобы у него
// был включен компонент физики (obj.physics.enabled) и коллайдер должен
// быть типа ColliderType::DYNAMIC. Последнее можно проверить, используя
// метод of_type для obj.collider.
//
...
// 3. Ограничить скорость, чтобы она не превышала максимальную скорость
// падения.
...
// (obj.physics.can_jump). В таком случае нужно придать ему скорость
// вертикально вверх. С конкретными значениями можно поиграться, но
// желательно, чтобы прыжок выглядел достаточно реалистично. И после
// придания скорости остаётся изменить флаг, отвечающий за возможность
// прыжка. Мы считаем, что игрок не может прыгать, пока он не находится на
// земле. Приземление в этой функции рассматривать не нужно. dt можно не
// использовать.
//
...
//
// Эта функция вызывается каждый кадр игры. Необходимо, чтобы камера
// следовала за объектом obj с некоторым запозданием. Для этого нужно
// вычислить вектор, по которому движется камера с помощью полей
// obj.position и ctx.camera_pos и сделать так, чтобы поле ctx.camera_pos
// передвигалась с некоторой скоростью вдоль этого вектора. Так как скорость
// указывается в метрах в секунду, а функция вызывается чаще, чем раз в
// секунду, необходимо изменять позицию камеры только на то расстояние,
// которое она могла пройти после предыдущего вызова функции. Время
// прошедшее с прошлого вызова передаётся в параметре dt
//
// Рекомендуемые функции для выполнения задания:
// - Vector2Length
//
...
// - CheckCollision
//
// При выполнении этого задания у вас есть возможность добавить свою
// текстуру финиша уровня Assets/finish.png
//
// Возможное решение может занимать примерно 6-7 строк.
// Ваше решение может сильно отличаться.
//
...
// Задание EnemyAI.
//
// Данная функция реализует искусственный интеллект для врагов в игровой
// сцене.
//
// Реализация достаточно свободная. Мы предлагаем следующее: противник будет
// преследовать игрока.
//
// Внутри функции определяется объект игрока путем вызова функции
// find_player, которая возвращает указатель на объект игрока в сцене.
//
// Затем функция должна вычислить разницу между позицией противника и
// позицией игрока, чтобы понять, в каком направлении нужно двигаться.
// Значение перемещения move вычисляется, умножая скорость движения врага на
// время, прошедшее с последнего кадра.
//
// Затем функция изменяет позицию врага, добавляя или вычитая значение move
// из его текущей позиции в зависимости от того, находится ли противник
// слева или справа от игрока.
//
// Рекомендуемые функции для выполнения задания:
// - find_player
//
// При выполнении этого задания у вас есть возможность добавить свою
// текстуру противника Assets/enemy1.png
//
...
// -
//   вправо.
// - Движение игрока определим 2D-вектором move, где каждая компонента
// принимает
//   значение из множества {-1, 0, 1}. Например, вектор {-1, 0} — будет
//   описывать движение влево. Тогда положение игрока меняется на основе
//   вектора move, умноженного на скорость игрока и время, прошедшее с
//   прошлого кадра.
//
// Рекомендуемые функции для выполнения задания:
// - IsKeyDown
...
// Задание ShootBullet.
//
// Данная функция вызывается при нажатии игроком на кнопку выстрела.
// Необходимо создать новый объект bullet = Object() и установить у него
// некоторые из полей. Так как снаряды запускает игрок, то начальная позиция
// пули bullet.position должна совпадать с позицией игрока. Картинка пули
// должна считываться из файла Assets/bullet.png. Для добавления её к
// объекту необходимо сконструировать новый объект Render и присвоить его
// полю bullet.render. Для создания физического тела пули необходимо
// сконструировать объект Collider и присвоить его полю bullet.collider. Для
// автоматического определения размеров тела по графике, конструктору нужно
// передать bullet.render. Чтобы игрок не сталкивался с пулей конструктору
// также нужно передать тип физического тела - ColliderType::EVENT.
//
// Далее необходимо указать параметры самой пули. Для этого можно
// использовать конструктор класса Bullet, в котором указать скорость пули и
// длительность её жизни. Так как пуля должна выпускаться в том же
// направлении, в которое смотрит игрок, скорость по оси x будет
// отрицательной, если player.player.direction равна Direction::LEFT, и
// положительной, если равна Direction::RIGHT. Время жизни пули необходимо
// указывать для того, чтобы она уничтожилась в функции UpdateBullet через
// указанное количество секунд, а не летела вечно.
//
...
// obj.position объекта. Так как скорость пули указывается в метрах в
// секунду, а функция вызывается чаще, чем раз в секунду, необходимо
// изменять позицию пули только на то расстояние, которое она могла
// пролететь после предыдущего вызова функции. Время прошедшее с прошлого
// вызова передаётся в параметре dt.
//
// Второй и третий параметры необходимо использовать для уничтожения пули
// через некоторое время после её запуска. Параметр obj.bullet.lifetime
// должен хранить время, прошедшее с запуска пули. Для этого при каждом
// вызове этой функции он должен изменяться на время, прошедшее с прошлого
// вызова. Как только obj.bullet.lifetime превысить obj.bullet.max_lifetime,
// необходимо уничтожить объект.
//
// Рекомендуемые функции для выполнения задания:
// - Destroy
//
...
// Задание KillEnemies.
//
// Пройдёмся в цикле по объектам сцены (ctx.current_scene), проверяя, что
// этот объект является врагом (obj.enemy.enabled). Если он не враг, то
// рассматривать его дальше не следует.
//
// Затем, внутри цикла, пройдёмся по всем объектам сцены ещё раз. На этот
// раз в поисках пуль. Если у объекта не установлено поле
// bullet.bullet.enabled, его можно не рассматривать дальше.
//
...
// будет применена функция. Функция предназначена для выполнения действий
// при смерти игрока или противника.
//
...
//
// Для реализации необходимо сначала загрузить звук из файла, используя
// функцию LoadSound, после чего воспроизвести его.
//
// Рекомендуемые функции для выполнения задания:
// - LoadSound
// - PlaySound
...
// созданы в игровом мире, и проигрывает звуковой эффект в соответствии с
// типом объекта. Если объект является пулей (obj.bullet.enabled),
...
//
// Данная функция выполняется когда игрок умер от прикосновения к
// противнику. В целом, реализовать эту функцию можно практически как
// угодно, но минимально допустимый вариант - текст, сообщающий о смерти
// игрока на фоне чёрного прямоугольника.
//
// Рекомендуемые функции для выполнения задания:
// - MeasureText
// - DrawRectangle
...
// угодно, но минимально допустимый вариант - текст, сообщающий о проигрыше
// на фоне чёрного прямоугольника.
//
// Рекомендуемые функции для выполнения задания:
// - MeasureText
// - DrawRectangle
...
// минимально допустимый вариант - текст, сообщающий о победе на фоне
// чёрного прямоугольника.
//
// Рекомендуемые функции для выполнения задания:
// - MeasureText
// - DrawRectangle
...
// Что можно добавить:
//
// Необходимо создать пустой объект bg, который будет соответствовать
// фоновой картинке. В поле bg.render записать новый объект Render, которая
// читает файл Assets/menu_background.png и задаёт ему размер окна
// ctx.screen_size.
//
// После этого этот объект нужно добавить в сцену через
// game_scene.push_back.
//
